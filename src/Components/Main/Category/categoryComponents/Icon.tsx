...
useEffect(() => {
  ...
  return (
    ...
    <div className="category-icons scroll-w-none">
      {res.map((el) => { return(
        <div key={el.id} className={`individual-category-icons w-[18%] inline-block mb-2.5 p-0 first:ml-[5px]`} onClick={() => {}}>
        <div className="icon-wrapper mb-0 h-[55px] p-0 text-center" onClick={() => navigate(`/${el.name}`)}> 
          <img alt={el.name} className="h-12.5 w-12.5 block m-[0_auto_6px]" src={el.img}></img>
